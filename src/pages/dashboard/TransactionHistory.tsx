...
import {FileText,Calendar,Tag,IdCard,DollarSign,CheckCircle,XCircle,Clock,
} from 'lucide-react';
...
const PaymentTransactionHistory: React.FC = () => {
  ...
  useEffect(() => {
    const storedTxns = localStorage.getItem('paymentTransactions');
    let allTxns: PaymentTransaction[] = [];

    if (storedTxns) {
      try { allTxns = JSON.parse(storedTxns); } catch { allTxns = []; }
    }

    const storedUser = localStorage.getItem('user');
    const currentUser = storedUser
      ? JSON.parse(storedUser) as { id: string; userType: string }
      : null;

    if (currentUser) {
      allTxns = allTxns.map(txn => ({
        ...txn,
        userId: txn.userId || currentUser.id,
      }));
      localStorage.setItem('paymentTransactions', JSON.stringify(allTxns));
    }

    const isAdmin = currentUser?.userType === 'admin' || currentUser?.userType === 'super_admin';
    const filtered = isAdmin
      ? allTxns
      : allTxns.filter(txn => txn.userId === currentUser?.id);
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-black transition-colors duration-300">
        <div className="max-w-6xl mx-auto px-4 py-10">
          ...
          {loading ? (
            ...
            <div className="text-center text-lg text-gray-600 dark:text-gray-400">No transactions found.</div>
          ) : (
            <>
                  ...
                  >
                    ...
                    <div className="mb-3 sm:mb-4 flex items-center">
                      ...
                      <h3 className="font-semibold text-gray-800 dark:text-gray-100 text-sm sm:text-base">
                        ...
                        <span className="text-base sm:text-lg text-indigo-600 dark:text-indigo-400">
                    ...
                    {/* Date & Time */}
                    <div className="mb-3 sm:mb-4 flex items-center text-xs sm:text-sm text-gray-600 dark:text-gray-400">
              ...
              <div className="mt-6 sm:mt-8 flex flex-wrap justify-center gap-1 sm:gap-2">
                <button
                  ...
                  className="px-2 sm:px-3 py-1 rounded border border-gray-400 dark:border-gray-600 bg-white dark:bg-gray-800 text-xs sm:text-sm text-gray-700 dark:text-gray-300 disabled:opacity-50"
                >
                  Prev
                </button>
                ...
                {Array.from({ length: totalPages }, (_, i) => (
                  <button
                    ...
                    className={`px-2 sm:px-3 py-1 rounded border text-xs sm:text-sm ${currentPage === i + 1
        ...
        <section
          className="relative w-full h-56 sm:h-64 overflow-hidden mb-8 sm:mb-12  mx-3 sm:mx-12 bg-gradient-to-r from-gray-800/90 via-gray-900/90 to-gray-800/90 dark:bg-gradient-to-r dark:from-black/80 dark:via-gray-800/80 dark:to-gray-700/80 p-5 sm:p-8 flex flex-col justify-center items-center text-center left-0 sm:-left-12"
        >
          ...
          <h2 className="text-xl sm:text-2xl font-bold mb-1.5 sm:mb-2">Ready to Make a New Payment?</h2>
          <p className="text-xs sm:text-sm md:text-base mb-4 sm:mb-6">Stay on top of your bills with quick, easy payments.</p>

          <button
            onClick={() => navigate('/PayBills')}
            className="relative z-10 px-6 sm:px-8 py-2.5 sm:py-3 rounded-full font-semibold bg-black text-white hover:bg-gray-800 dark:bg-white dark:text-black dark:hover:bg-gray-100 transition text-xs sm:text-sm md:text-base"
          >
            Make a Payment
          </button>
        </section>
